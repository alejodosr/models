...
ROOTDIR = "/media/alejandro/DATA/datasets/simple_uav_dataset/simulation_divided/eval"
OUTPATH = "/media/alejandro/DATA/datasets/simple_uav_dataset/simulation_divided/uav_eval.record"
    ...
    for subdir, dirs, files in os.walk(ROOTDIR):
        for file in files:
            if file.endswith(".jpg") and not file.startswith("frame_seg"):
                ...
                width = w  # Image width
                filename = tf.compat.as_bytes(file)  # Filename of the image. Empty if image is not from file
                encoded_image_data = tf.compat.as_bytes(cv2.imencode('.jpg', img)[1].tostring())  # Encoded image bytes
                image_format = b'jpg'  # b'jpeg' or b'png'

                xmins = [float(x) / float(w)]  # List of normalized left x coordinates in bounding box (1 per box)
                xmaxs = [float(x + w) / float(w)]  # List of normalized right x coordinates in bounding box
                # (1 per box)
                ymins = [float(y) / float(h)]  # List of normalized top y coordinates in bounding box (1 per box)
                ymaxs = [float(y + h) / float(h)]  # List of normalized bottom y coordinates in bounding box
                # (1 per box)
